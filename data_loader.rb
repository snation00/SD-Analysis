...
module DataLoader
  autoload :HssSim, 'data_loader/hss_sim'
  ...
  class << self
    ...
    def process_new_ugly_error_points(file_name)
      ...
      prev_int = 94.99e-9
      new_zero = 0.0 
      file_config = { :cycle_start => 100e-9, :time_increment => 3.2e-9 }
      ...
      lines.each do |line|
        ...
        if (prev_point[:voltage] < 0.0 && v >= 0.0)
          ...
          if (jitter >= 670e-12 or jitter <= 610e-12 and t >= 9.9e-8)
            wc = HssSim.create(circuit_info.merge({ :period => jitter, :jitter_time => t }))
        ...
        if (prev_voltage >= new_zero && v < new_zero)
          if max[:bit_miss_v] != new_zero && max[:bit_miss_v] < 0.9 
            #For neg: 50.5   For pos: 49.95
            #max[:bit_miss_v] = (max[:miss_time] - t).abs()
            hs = HssSim.create(circuit_info.merge(max))
        ...
        if (prev_voltage < new_zero && v >= new_zero)
          if min[:bit_miss_v] != new_zero && min[:bit_miss_v] > -0.9 
            #For neg: 23.8   For pos: 27.16
            #min[:bit_miss_v] = (min[:miss_time] - t).abs()
            HssSim.create(circuit_info.merge(min))
          end
          max[:bit_miss_v] = new_zero; max[:miss_time] = 0.0
        end
        prev_voltage = v
